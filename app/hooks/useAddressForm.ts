...
export default function useAddressForm() {
  ...
  useEffect(() => {
    ...
    const filterAddresses = () => {
      setFilteredAddresses(
        data.filter((address) =>
          `${removeExtraChars(getAddress(address['LAHIAADRESS']))}, ${address['SIHTNUMBER']}, ${
            address['TAISAADRESS'].split(', ')[1]
          }, ${address['TAISAADRESS'].split(', ')[0]}`
            .toLowerCase()
            .startsWith(removeExtraChars(values.address).trim().toLowerCase()),
        ),
      );
    };
  ...
  const filterApartments = () => {
    return allAddressApartments?.filter((apartment) =>
      apartment.startsWith(values.apartment.trim()),
    );
  };
  ...
  const handleApartmentInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setValues((prevState) => ({
      ...prevState,
      apartment: removeExtraSpaces(e.target.value).trimStart(),
    }));
  ...
  const handleAddressClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    setSelected({
      ...selected,
      address:
        (e.target as HTMLAnchorElement).getAttribute('data-search-item')?.split(',')[0] ?? '',
    });
  ...
  const handleApartmentClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    setSelected({
      ...selected,
      apartment: (e.target as HTMLAnchorElement).getAttribute('data-search-item') ?? '',
    });
    setErrors({ ...errors, apartment: '' });
  };
  ...
  const addresses = filteredAddresses.map(
    (address) =>
      `${address['LAHIAADRESS']}, ${address['SIHTNUMBER']}, ${
        address['TAISAADRESS'].split(', ')[1]
      }, ${address['TAISAADRESS'].split(', ')[0]}`,
  );
   ...
   */
  const allAddressApartments =
    selected.address === ''
      ? []
      : data
          .filter(
            (address) =>
              address['LAHIAADRESS'].match(new RegExp(`${selected.address}-.*`, 'g'))?.[0],
          )
