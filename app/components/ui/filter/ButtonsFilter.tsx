    ...
    return (
        <div className="flex flex-wrap gap-2">
            {Object.keys(buttons).map(button =>
                <Button key={button} handleClick={handleFilterClick} variant={usedFilters[button] ? 'primary' : 'secondary'} size="lg" className="!w-max rounded-md uppercase" name={button}>{button}</Button>
            )}
        </div>
    )
}