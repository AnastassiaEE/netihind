...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      jsonb_agg(
        ...
        ORDER BY CASE WHEN pe.equipment_id IS NOT NULL THEN et_from_equipment.name ELSE et_from_provider.name END
  ...
  FROM addresses a
    ...
    LEFT JOIN internet_package_equipment ec
        ON ip.id = ec.package_id
    ...
    LEFT JOIN internet_package_discounts ipd
        ON ipd.package_id = ip.id
    LEFT JOIN internet_package_discount_campaigns ipdc
        ON ipdc.package_id = ip.id
    LEFT JOIN discount_campaigns dc
        ON dc.id = ipdc.discount_campaign_id AND dc.is_active = true
