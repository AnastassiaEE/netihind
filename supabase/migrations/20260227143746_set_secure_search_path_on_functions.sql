...
CREATE OR REPLACE FUNCTION public.get_providers_by_address(p_oid text)
...
BEGIN
    ...
    INNER JOIN public.address_technologies at
        ON a.ads_oid = at.address_ads_oid
    INNER JOIN public.providers p
        ON at.provider_id = p.id
...
CREATE OR REPLACE FUNCTION public.get_technologies_by_address(p_oid text)
RETURNS TABLE(id smallint, name text)
LANGUAGE plpgsql
SET search_path = ''
AS $$
BEGIN
    ...
    INNER JOIN public.address_technologies at
        ON a.ads_oid = at.address_ads_oid
    INNER JOIN public.technologies t
        ON at.technology_id = t.id
    INNER JOIN public.technology_types tt
...
CREATE OR REPLACE FUNCTION public.get_internet_packages_by_address(
  p_oid TEXT,
  p_sort TEXT,
...
BEGIN
  ...
  SELECT
    ...
    CASE 
        WHEN MAX(ipd.price) IS NULL THEN NULL
        ELSE jsonb_build_object(
            'price', MAX(ipd.price),
            'duration', MAX(ipd.duration)
        )
    END AS discount
  ...
  FROM public.addresses a
    INNER JOIN public.address_technologies at
        ON a.ads_oid = at.address_ads_oid
    INNER JOIN public.internet_packages ip
    ...
    INNER JOIN public.technology_types tt
        ON t.technology_type_id = tt.id
    INNER JOIN public.connection_types ct
        ON tt.connection_type_id = ct.id
    INNER JOIN public.providers p
        ON ip.provider_id = p.id
    LEFT JOIN public.infrastructure_providers i
        ON ip.infrastructure_provider_id = i.id
    LEFT JOIN public.provider_installations pi
        ON pi.provider_id = p.id
    LEFT JOIN public.internet_package_discounts ipd
        ON ipd.package_id = ip.id
    ...
    LEFT JOIN LATERAL (
        ...
        FROM public.internet_package_equipment ec
        LEFT JOIN public.equipment_combination_items eci
            ON ec.equipment_combination_id = eci.equipment_combination_id
        LEFT JOIN public.provider_equipment pe
            ON eci.provider_equipment_id = pe.id
        LEFT JOIN public.equipment e
            ON pe.equipment_id = e.id
        LEFT JOIN public.equipment_types et_from_equipment
            ON e.equipment_type_id = et_from_equipment.id
        LEFT JOIN public.equipment_types et_from_provider
        ...
        LEFT JOIN LATERAL (
            ...
            FROM public.equipment_payments ep
            LEFT JOIN public.payment_options po
    ...
    LEFT JOIN LATERAL (
        ...
        FROM public.internet_package_discount_campaigns ipdc
        JOIN public.discount_campaigns dc
  ...
  ORDER BY
    ...
    CASE 
        WHEN p_sort = 'price_asc' THEN COALESCE(MAX(ipd.price), ip.price)
    END ASC,

    CASE
        WHEN p_sort = 'price_desc' THEN COALESCE(MAX(ipd.price), ip.price)
    END DESC,

    CASE
        WHEN p_sort = 'default'
        OR p_sort NOT IN ('default', 'speed_desc', 'price_asc', 'price_desc')
        THEN ip.provider_id
    END ASC,
    ...
    CASE
        WHEN p_sort = 'default'
        OR p_sort NOT IN ('default', 'speed_desc', 'price_asc', 'price_desc')
        THEN COALESCE(NULLIF(ip.download_speed, -1), 999999)
    END DESC,
...
CREATE OR REPLACE FUNCTION public.get_string_translations()
RETURNS TABLE(
    id smallint,
    original text,
    et text,
...
BEGIN
    ...
    FROM public.string_translations s;
END;
$$;